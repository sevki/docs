...
async function handleRequest(event) {
  ...
  if (!response) {
    const doc = await getmd(u.pathname);
    let rendered = ReactDOMServer.renderToString(
      <StyleProvider renderer={renderer}>
        <MarkdownPage input={doc} />
      </StyleProvider>
    );
...
async function getmd(name) {
  const url = "https://api.github.com/repos/sevki/docs/contents/docs" + name;
  try {
    const contentsResp = await fetch(url, { headers });
    const contents = await contentsResp.json();
    console.log(contents);
    const file = await fetch(contents.download_url, {
      headers
    });
